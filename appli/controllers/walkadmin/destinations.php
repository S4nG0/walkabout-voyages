...
class Destinations  extends CI_Controller{
    ...
    public function index($page = 1){
        ...
        $config['base_url'] = base_url().'walkadmin/destinations/';
        $config['total_rows'] = $count;// faire attention taille totale
        $nb_destinations = $config['per_page'] = 10;
        $config['num_links'] = 3;
        ...
        $start = ($page*$nb_destinations)-$nb_destinations;
        $data['destinations']=$this->destination->getAllDestinationPagination($start, $nb_destinations);
    ...
    public function recherche($page = 1){
        ...
        $this->db->or_like('destination.titre',$data['search']);
        $this->db->or_like('destination.ville',$data['search']);
        $this->db->or_like('pays.nom',$data['search']);
        $this->db->from('destination');
        $this->db->join('pays','pays.idPays=destination.idPays');
        $this->db->where('active','true');
        $count = $this->db->count_all_results();
        /*Load des helpers et librairies*/
        $this->load->library('pagination');
        /*Parametrage de la pagination*/
        $config['base_url'] = base_url().'walkadmin/destinations/recherche/';
        $config['total_rows'] = $count;// faire attention taille totale
        $nb_destinations = $config['per_page'] = 10;
        $config['num_links'] = 3;
        $config['use_page_numbers'] = true;
        ...
        /*Création des variables de selection des carnets*/
        $start = ($page*$nb_destinations)-$nb_destinations;
        $data['destinations']=$this->destination->getAllDestinationSearchPagination($data['search'],$start, $nb_destinations);
        $data['admin'] = $this->session->userdata('admin');
        $this->load->view('wadmin/template/header', $data);
        $this->load->view('wadmin/template/menu', $data);
        $this->load->view('wadmin/pages/Destinations/recherche',$data);
        $this->load->view('wadmin/template/footer');
